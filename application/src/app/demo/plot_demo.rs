...
impl Default for LineDemo {
    fn default() -> Self {
        Self {
            ...
            throttle_trace: Default::default(),
            brake_trace: Default::default()
        }
    }
}
...
impl LineDemo {
    ...
    fn sin(&self) -> Line {
        let time = self.time;
        Line::new(
            PlotPoints::new(self.throttle_trace.iter().rev().take(600).rev().enumerate().map(|(index, point)| {
                [(index as f64 -  min(600, self.throttle_trace.len()) as f64) / 60.0, *point as f64]
            }).collect::<Vec<[f64; 2]>>())
        )
    ...
    fn brake(&self) -> Line {
        let time = self.time;
        Line::new(
            PlotPoints::new(self.brake_trace.iter().rev().take(600).rev().enumerate().map(|(index, point)| {
                [(index as f64 - min(600, self.brake_trace.len()) as f64) / 60.0, *point as f64]
            }).collect::<Vec<[f64; 2]>>())
        )
...
impl LineDemo {
    fn ui(&mut self, ui: &mut Ui) -> Response {
        ...
        plot = plot.include_x(-10.0);
        plot = plot.view_aspect(5.0);
        plot = plot.include_y(1.1);
...
impl super::Demo for PlotDemo {
    ...
    fn show(&mut self, ctx: &Context, open: &mut bool) {
        ...
        Window::new(self.name())
            .open(open)
            .default_size(vec2(1200.0, 500.0))
            .vscroll(false)
